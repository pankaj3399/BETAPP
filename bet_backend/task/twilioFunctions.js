...
const sendMessage = async (req, resp) => {
  try {
    ...
    resp
      .status(200)
      .json({ message: "Message sent successfully", messageSid: message.sid });
  } catch (error) {
...
const sendResolutionUpdate = async (req, resp) => {
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messagingServiceSid: serviceId,
      sendAt: new Date(
        Date.UTC(utcYear, utcMonth - 1, utcDay, utcHours, utcMin, utcSec)
      ),
      scheduleType: "fixed",
    ...
    resp
      .status(200)
      .json({
        message: `Message scheduled successfully for ${req.body.resolDate}`,
